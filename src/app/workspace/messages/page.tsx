...
export default function MessagesPage() {
  ...
  return (
    ...
    >
      ...
      {/* ─── CENTER AREA (CHAT) ─── */}
      <motion.div variants={itemVariants} className={`flex-1 flex flex-col ${glassPanelClass}`}>
        {selectedUser ? (
          <>
            {/* Chat Header */}
            ...
            {/* Messages Area */}
            <div ref={scrollContainerRef} className="flex-1 overflow-y-auto px-6 py-6 custom-scrollbar flex flex-col gap-5 z-0 relative">
              <div className="absolute inset-0 opacity-[0.03] pointer-events-none mix-blend-overlay" style={{ backgroundImage: 'url("data:image/svg+xml,%3Csvg viewBox=%220 0 200 200%22 xmlns=%22http://www.w3.org/2000/svg%22%3E%3Cfilter id=%22noiseFilter%22%3E%3CfeTurbulence type=%22fractalNoise%22 baseFrequency=%220.65%22 numOctaves=%223%22 stitchTiles=%22stitch%22/%3E%3C/filter%3E%3Crect width=%22100%25%22 height=%22100%25%22 filter=%22url(%23noiseFilter)%22/%3E%3C/svg%3E")' }} />
              
              {messages.length === 0 ? (
                <div className="m-auto flex flex-col items-center justify-center max-w-sm text-center">
                  ...
                  <h3 className="text-[18px] font-semibold text-white mb-2 tracking-tight">Start the Conversation</h3>
                  <p className="text-[13px] text-slate-400 leading-relaxed">
                    Send a message to {selectedUser.displayName.split(" ")[0]} to initiate a secure, realtime connection via Firestore.
                  </p>
                </div>
              ) : (
